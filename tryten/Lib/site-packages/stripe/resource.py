...
class Account(CreateableAPIResource, ListableAPIResource,
    ...
    def _build_instance_url(cls, sid):
        ...
        sid = util.utf8(sid)
        base = cls.class_url()
        extn = urllib.parse.quote_plus(sid)
        return "%s/%s" % (base, extn)
    ...
    def modify_external_account(cls, sid, external_account_id, **params):
        url = "%s/%s/external_accounts/%s" % (
            cls.class_url(), urllib.parse.quote_plus(util.utf8(sid)),
            urllib.parse.quote_plus(util.utf8(external_account_id)))
        return cls._modify(url, **params)
...
class Customer(CreateableAPIResource, UpdateableAPIResource,
    ...
    def modify_source(cls, sid, source_id, **params):
        url = "%s/%s/sources/%s" % (
            cls.class_url(), urllib.parse.quote_plus(util.utf8(sid)),
            urllib.parse.quote_plus(util.utf8(source_id)))
        return cls._modify(url, **params)
...
class Reversal(UpdateableAPIResource):
    ...
    def instance_url(self):
        ...
        cust_extn = urllib.parse.quote_plus(transfer)
        extn = urllib.parse.quote_plus(token)
...
class ApplicationFeeRefund(UpdateableAPIResource):
    ...
    def _build_instance_url(cls, fee, sid):
        ...
        cust_extn = urllib.parse.quote_plus(fee)
        extn = urllib.parse.quote_plus(sid)
...
class Source(CreateableAPIResource, UpdateableAPIResource, VerifyMixin):
    def delete(self, **params):
        if hasattr(self, 'customer') and self.customer:
            extn = urllib.parse.quote_plus(util.utf8(self.id))
            customer = util.utf8(self.customer)
            base = Customer.class_url()
            owner_extn = urllib.parse.quote_plus(customer)
